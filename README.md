...
Raw run committed at `backtest/results/runs/2020-01-01_2025-01-01_weekly/` (equity curve + metrics; summary also mirrored at `backtest/results/sample_metrics.json` for the dashboards' fallback view), reproducible with `python -m backtest.engine --start 2020-01-01 --end 2025-01-01`.
...
### Live Paper Trading
Running daily via the scheduled GitHub Actions job (`.github/workflows/paper-trading.yml`) against a funded Alpaca paper account, logging every order, fill, and portfolio snapshot to Postgres (`live/storage.py`). `live/` (scheduler, Alpaca client) is unit-tested with all network calls mocked, and the live path itself has been run and verified end-to-end. There isn't yet enough accumulated history for a meaningful live-vs-backtest performance table (that needs weeks/months of daily snapshots, not days) — track it live on either dashboard, which plot both series indexed to % return from their own starting point so a $100K paper account and a $1M backtest are directly comparable.
...
Both read live state from Postgres (`DATABASE_URL`) and backtested equity/metrics from `backtest/results/` (via `backtest/results_io.py`), and both degrade to an empty-state message rather than crashing when live data doesn't exist yet (e.g. before the scheduler's first run, or before `DATABASE_URL` is configured locally). The per-holding factor score breakdown is intentionally left to the research notebook rather than either dashboard (see the Factor Breakdown tab), since it requires a fresh factor computation and isn't cached anywhere either dashboard could read cheaply.
